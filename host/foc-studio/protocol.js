...
export function parseTelemetry(line) {
  ...
  const [state, fault, velocity, current, position, busVoltage, temperature,
    ...
    anticoggingPostprocessIndex = 0, anticoggingRejectedEstimatorSamples = 0] = values;
  return {
    ...
    anticoggingRejectedEstimatorSamples,
  };
}
...
export function parseFastTelemetry(line) {
  ...
  const [state, velocity, current, position, busVoltage, phaseAVoltage,
    ...
    abzObserverMinBandwidth = 0, abzVelocityTorqueLimit = 0,
  ...
  return {
    axisState: AXIS_STATE[state] ?? `状态 ${state}`,
    stateCode: state,
    // Canonical names first, legacy aliases kept for older consumers.
    controlVelocity: velocity,
    velocity,
    encoderPllVelocity: rawVelocity,
    rawVelocity,
    velocityWindow50ms: windowVelocity,
    windowVelocity,
    velocityWindow100ms,
    mtVelocity: mTVelocity,
    mTVelocity,
    observerVelocity: controlObserverVelocity,
