...
bool Motor::arm() {
    ...
    // later re-arm after a fault must abort it instead of leaving the UI with
    // a calibration that is active but no longer running.
    const bool entering_cogging_calibration =
            axis_->controller_.config_.anticogging.calib_anticogging &&
            axis_->requested_state_ == Axis::AXIS_STATE_CLOSED_LOOP_CONTROL;
    axis_->controller_.reset(!entering_cogging_calibration);
