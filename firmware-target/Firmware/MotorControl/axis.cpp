...
bool Axis::watchdog_check() {
    ...
    // Idle is the safe, disarmed state. A host is not required to keep
    // sending control frames while the axis is stopped, so do not latch a
    // communication fault until the axis is actually running.
...
bool Axis::run_lockin_spin(const LockinConfig_t &lockin_config) {
    ...
    if (!check_for_errors() || requested_state_ != AXIS_STATE_UNDEFINED) {
...
bool Axis::run_closed_loop_control_loop() {
    closed_loop_control_active_ = false;
    ...
    if (controller_.config_.circular_setpoints) {
        if (!controller_.pos_estimate_circular_src_) {
            ...
            // A protocol command can arrive after current_state_ changes but
            // before this handler runs. Do not overwrite that fresh target.
            if (!controller_.input_pos_updated_)
    ...
    closed_loop_control_active_ = true;
    run_control_loop([this](){
        // Note that all estimators are updated in the loop prefix in run_control_loop
        float torque_setpoint;
        if (!controller_.update(&torque_setpoint))
    ...
    for (;;) {
        ...
        switch (current_state_) {
            ...
            case AXIS_STATE_SENSORLESS_CONTROL: {
                if (!motor_.is_calibrated_ || motor_.config_.direction==0)
                        goto invalid_state_label;
                // Each spin-up must start from a clean flux/PLL state. Stale
                // observer memory can turn the open-loop to FOC handoff into a
                // current-limit transient.
                sensorless_estimator_.reset();
