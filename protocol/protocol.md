...
0.004～0.018 Nm 的平滑斜坡，并按正向速度误差提供限幅助推和低速保持项；0.5 turn/s 以下允许使用扩展上限。越齿恢复后，0.5～1.0 turn/s 保持 0.014 Nm 的运行基准，随后在 1.0～1.5 turn/s 线性退回 0.004 Nm，避免电机刚越齿又因补偿回落而停转；控制器的低速淡出保证该保持项不进入正常的 1.5～2.0 turn/s 区间。只有编码器累计前进且实测速度持续达到目标的 55% 后才退出脱困，避免低速越齿后过早卸载。补偿在 1.0～2.0 turn/s 之间逐步退出；ABZ 积分转矩在
