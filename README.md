...
FOC Studio 是一个面向 ODrive V3.6 和 3505-KV650 电机的单轴 FOC 项目。
当前使用 M0 轴，支持以下反馈方式：
...
Windows 上位机通过 ODrive 的 USB CDC 接口通信，默认作为独立 Electron 软件运行，
也保留 Microsoft Edge/Chrome 浏览器备用方式。原始源码目录
`D:\Odrive\程序源码\ODrive-fw-v0.5.1\ODrive-fw-v0.5.1` 没有被修改。
...
内置 Web Serial 访问 ODrive USB CDC，不需要浏览器页面。
...
- `odrive-baseline/Firmware/build/ODriveFirmware.elf`
- `odrive-baseline/Firmware/build/ODriveFirmware.hex`
- `odrive-baseline/Firmware/build/ODriveFirmware.bin`

目标处理器架构为 ARMv7E-M，STM32 Flash 起始地址为 `0x08000000`。

## 目录结构

- `odrive-baseline/Firmware`：可重复编译的 ODrive v0.5.1 STM32 固件工程，
  已加入产品默认参数和 FOC Studio ASCII 命令；
- `firmware`：提取后的便携配置、安全状态机和保留的 ODrive 控制源码；
...
因为这些内容是生成可验证 STM32 固件所必需的。原始 ODrive GUI、无关主机工具和
...
3505 电机额定电压是 12 V。ODrive 板可以承受 56 V，不代表这台电机可以使用
