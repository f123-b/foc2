...
static void load_foc_studio_defaults() {
    // Axis 0 is the only populated axis in the FOC Studio product profile.
    Motor::Config_t& motor = motor_configs[0];
    motor.pre_calibrated = false;
    motor.pole_pairs = 10;
    motor.calibration_current = 1.0f;
    motor.resistance_calib_max_voltage = 1.0f;
    motor.phase_resistance = 0.1f;
    motor.phase_inductance = 42.3e-6f;
    motor.torque_constant = 8.27f / 650.0f;
    motor.direction = 0;
    motor.current_lim = 2.0f;
    motor.current_lim_margin = 1.0f;
    motor.torque_lim = 0.2f;
    motor.current_control_bandwidth = 500.0f;

    Encoder::Config_t& encoder = encoder_configs[0];
    encoder.mode = Encoder::MODE_SPI_ABS_AMS;
    encoder.cpr = 16384;
    encoder.abs_spi_cs_gpio_pin = 4;
    encoder.bandwidth = 100.0f;
    encoder.use_index = false;
    encoder.pre_calibrated = false;

    sensorless_configs[0].pm_flux_linkage = 5.51328895422f / (10.0f * 650.0f);

    Axis::Config_t& axis = axis_configs[0];
    axis.enable_watchdog = true;
    axis.watchdog_timeout = 1.0f;
    axis.sensorless_ramp.current = 1.5f;
    axis.sensorless_ramp.ramp_time = 1.5f;
    axis.sensorless_ramp.ramp_distance = 2.0f * M_PI;
    axis.sensorless_ramp.accel = 100.0f;
    axis.sensorless_ramp.vel = 400.0f;
    axis.sensorless_ramp.finish_on_vel = true;
    axis.sensorless_ramp.finish_on_distance = false;
    axis.sensorless_ramp.finish_on_enc_idx = false;

    Controller::Config_t& controller = controller_configs[0];
    controller.pos_gain = 0.8f;
    controller.vel_gain = 0.0015f;
    controller.vel_integrator_gain = 0.005f;
    controller.vel_limit = 20.0f;
    controller.vel_ramp_rate = 0.3f;
    controller.torque_ramp_rate = 0.01f;

    TrapezoidalTrajectory::Config_t& trajectory = trap_configs[0];
    trajectory.vel_limit = 0.30f;
    trajectory.accel_limit = 0.60f;
    trajectory.decel_limit = 0.60f;

    // This motor is rated for a 12 V system. Refuse operation on a high-voltage bus.
    odrv.config_.dc_bus_overvoltage_trip_level = 16.0f;
    odrv.config_.enable_ascii_protocol_on_usb = true;
}
